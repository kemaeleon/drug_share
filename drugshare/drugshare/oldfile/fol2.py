...
pop['2020']= pop['2020'].str.replace(",","").astype(float)/100000
...
for single_date in daterange(start_date, end_date):
    ...
    sds[date]=np.sqrt(sds[date])/covid_uk['2020']
    th = sds[date].max()
    ...
    for day in ref_back(single_date, 7):
        ...
        for index, row in rsds.iterrows():
            ...
            if float(row[date]) < -(np.sqrt(3)):
                ...
                folium.Marker(
                location = [pt[0].coords[0][1], pt[0].coords[0][0]],
                popup='Decreased by ' + '{:.3i}'.format(value),
